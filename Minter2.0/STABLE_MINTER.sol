...
contract USDFI_MINTER is Pausable, ReentrancyGuard {
        ...
        if (lastTriggerTime < block.timestamp.add(600)) {
            if (stabilityCheck.isStabilityOK() == true) {
                ...
                while (i < receiver.length) {
                    ...
                    USDFI(Token).mint(receiver[i], bal);
                    i += 1;
                }
            } else {
                uint256 i = 0;
                while (i < receiverPanic.length) {
                    ...
                    createdTokens = createdTokens.add(bal);
                    USDFI(Token).mint(receiverPanic[i], bal);
                    i += 1;
                }
            }
            lastTriggerTime = block.timestamp;
    ...
    // Set the Percent who receives the new Coins in Regular Mode.
    ...
    // Set the Percent who receives the new Coins in Panic Mode.
