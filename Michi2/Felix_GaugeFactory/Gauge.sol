...
contract Gauge is ReentrancyGuard {
    ...
    function depositWithNewRef(uint256 amount, address sponsor) external {
        _deposit(amount, msg.sender);
        _newRef(msg.sender, sponsor);
    }
    ...
    function depositForWithNewRef(
        uint256 amount,
        address account,
        address sponsor
    ) external {
        require(msg.sender == zapContract, "deposit(Gauge): only zap");
        _deposit(amount, account);
        _newRef(account, sponsor);
    }
    ...
    function _newRef(address account, address sponsor) internal {
        if (
            IReferrals(referralContract).isMember(account) == false &&
            IReferrals(referralContract).isMember(sponsor) == true
        ) {
            IReferrals(referralContract).addMember(account, sponsor);
        }
    }
