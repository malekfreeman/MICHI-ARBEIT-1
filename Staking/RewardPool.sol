...
contract RewardPool is Manager, ReentrancyGuard {
    ...
    constructor(address _stakingCoinAddress, address _rewardCoinAddress, address _vaultAddress) public {
    ...
    function emergencyWithdraw(uint256 _amount) public override updateReward(msg.sender) nonReentrant {
        require(_amount > 0, "Cannot withdraw 0");
    ...
    function getReward() public updateReward(msg.sender) nonReentrant {
        ...
        while (i < refLevelReward.length) {
            IERC20(rewardCoinAddress).safeTransfer(sponsor, refReward.div(100000).mul(refLevelReward[i]));
    ...
    function notifyRewardAmount() internal updateReward(address(0)){
        ...
        IERC20(rewardCoinAddress).safeTransferFrom(vaultAddress, address(this), reward);
    ...
    function remainingTimeToWithdraw(address _account) public view returns (uint256) {
    ...
    function remainingTimeToLock(address _account) public view returns (uint256) {
        uint256 endTime = requestedWithdrawTime[_account].add(lockTime).add(freeTime);
        if (block.timestamp < endTime) {
        return endTime.sub(block.timestamp);
         } else { 
             return 0;
    ...
    function canWithdraw(address _account) public view returns (bool) {
        if (block.timestamp > requestedWithdrawTime[_account].add(lockTime) && block.timestamp < requestedWithdrawTime[_account].add(lockTime).add(freeTime)) {
        return true;
         } else { 
             return false;
         }
